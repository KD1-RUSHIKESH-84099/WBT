...
)
{
    ...
    const body=new FormData()//use form data to upload multi-part file
    ...
    try{
        ...
        const response= await axios.post(`${config.serverUrl}/property/`,body,{headers:{token},})
    ...
    export  async function getProperties()
    {
        try{
            const token=sessionStorage.token
          const response=await axios.get(`${config.serverUrl}/property`,{headers:{token},
        })
        return response.data
        }
        catch(ex)
        {
            console.log(`exceptiom`,ex)
        }
        return null
    }
